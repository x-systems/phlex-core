...
## Handling Exceptions in Phlex Data and Phlex UI
...
The other option is to use automatic exception catching, ({php:attr}`\Phlex\Ui\App::catchExceptions`)
which will automatically catch any unhandled exception then pass it to {php:meth}`\Phlex\Ui\App::caughtException()`.

If you do not instantiate App, or set it up without automatic exception catching:

```
$app = new \Phlex\Ui\App(['catchExceptions' => false]);
```
