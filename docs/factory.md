...
```
$button = new \Phlex\Ui\Button('A Label');
$button->icon = new \Phlex\Ui\Icon('book');
...
This problem is solved in Phlex with "Seeds".
...
Phlex only uses setters/getters when they make sense. Argument like "icon" is a very
...
So here are some rules for Phlex and add-ons:
...
$defaults will only be sed if object implement InjectableTrait.
...
by using {php:meth}`InjectableTrait::setDefaults`.
...
It will then execute {php:meth}`InjectableTrait::setDefaults` with the
`['icon' => 'cake']` which will change value of $icon to `cake`.

The `cake` will be the final value of the example above. Even though `doInitialize()`
method is set to change the value of icon, the `doInitialize()` method is only executed
...
There are several ways to use Seed Merging and Phlex UI / Phlex Data makes use
...
```
$app = new \Phlex\Ui\App('Hello World');
...
Phlex UI is using form field classes from namespace \Phlex\Ui\Form\Control.
...
```
$seed = Factory::mergeSeeds($decorator, $field->ui, $inferred, [\Phlex\Ui\Form\Control\Line::class, 'form' => $this]);
$decorator = Factory::factory($seed, null, 'Form\Control');
```
